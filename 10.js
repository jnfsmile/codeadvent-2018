...
const utInput = parseInput(`position=< 9,  1> velocity=< 0,  2>
...
const ut1 = (input => getSquareSize(input))(utInput);
ut1;
assert(ut1.x === 21);
assert(ut1.y === 15);
const ut2 = (input => findIterationsForMinSquareSize(input))(utInput);
assert(ut2 === 3);

//printMessage(utInput, 3);

function printMessage(data, sec) {
  let stars = data.slice();
  stars = iterateStars(stars, sec);
  let sq = getSquareSize(stars);
  sq;
  stars
  let screen = new Array(sq.y + 1);
  for (let i = 0; i < screen.length; i++) {
    screen[i] = new Array(sq.x + 1).fill(".").join("");
  }
  stars.forEach(el => {
    let x = el.pos[0] - sq.minX;
    let y = el.pos[1] - sq.minY;

    screen[y] = screen[y].slice(0, x) + "#" + screen[y].slice(x + 1);
  });
  let print = "";
  for (let i = 0; i < screen.length; i++) {
...
function findIterationsForMinSquareSize(data) {
  let movement = data.slice();
  let size = { x: Infinity, y: Infinity };
  let i = -1;
  while (i++ < 1000000) {
    movement = iterateStars(movement);
    let sq = getSquareSize(movement);
    if (sq.x <= size.x && sq.y <= size.y) size = sq;
    else break;
  }
  return i;
}

function iterateStars(data, steps = 1) {
  for (let j = 0; j < data.length; j++) {
    let cur = data[j];
    data[j] = { v: cur.v };
    data[j].pos = [
      cur.pos[0] + steps * cur.v[0],
      cur.pos[1] + steps * cur.v[1]
    ];
  }
  return data;
}

function getSquareSize(data) {
  let x = (y = 0);
...
const input = parseInput(`position=<-20620, -41485> velocity=< 2,  4>
...
let r = findIterationsForMinSquareSize(input);
r;
printMessage(input, r);
