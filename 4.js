const utInput = `[1518-11-01 00:00] Guard #10 begins shift
...
const utInput2 = `[1518-11-01 00:00] Guard #10 begins shift
[1518-11-01 00:05] falls asleep
[1518-11-04 00:36] falls asleep
[1518-11-04 00:46] wakes up
[1518-11-05 00:03] Guard #99 begins shift
...
let ut1 = ((input, id) => guardRecords(input, id))(utInput, 10);
assert(ut1.length === 8);

let ut2 = ((input, id) => guardTotalSleep(input, id))(
  guardRecords(utInput, 10),
  10
);
assert(ut2 === 50);

let ut3 = ((input, id) => guardRecords(input, id))(sortByDate(utInput2), 10);
assert(ut3.length === 8);
let ut4 = ((input, id) => guardTotalSleep(input, id))(
  guardRecords(sortByDate(utInput2), 10),
  10
);
assert(ut4 === 50);
...
function maxSleeper(records) {
  let max = { time: 0, id: 0 };
  for (id of getIds(records)) {
    const time = guardTotalSleep(guardRecords(records, id));
    if (time > max.time) {
      max = { time, id };
    }
  }
...
function getIds(input) {
  ...
  input.forEach(element => {
    if (element.includes("#")) {
      ids.add(
        Number(
          element.substring(element.indexOf("#") + 1, element.indexOf(" b"))
        )
      );
    }
  });
...
function guardRecords(recordsOrig, id) {
  let records = [];
  let keep = false;
  for (let i = 0; i < recordsOrig.length; i++) {
    const record = recordsOrig[i];
    const curId = Number(
      record.substring(record.indexOf("#") + 1, record.indexOf(" b"))
    );
    if (record.includes("#")) {
      keep = id === curId;
    }
...
function sortByDate(records) {
  ...
  sortedRecords.sort((a, b) => {
    if (
      new Date(a.substring(a.indexOf("[") + 1, a.indexOf("]"))) <
      new Date(b.substring(b.indexOf("[") + 1, b.indexOf("]")))
    )
      return -1;
    if (
      new Date(a.substring(a.indexOf("[") + 1, a.indexOf("]"))) >
...
function guardTotalSleep(records) {
  let start,
    total = 0;
  for (let i = 0; i < records.length; i++) {
    let record = records[i];
    if (record.includes("sleep")) {
      start = Number(
        record.substring(record.indexOf(":") + 1, record.indexOf("]"))
      );
    } else if (record.includes("wake")) {
      total +=
        Number(record.substring(record.indexOf(":") + 1, record.indexOf("]"))) -
        start;
    }
  }
  return total;
